...
function buildSystemPrompt(goal: string, resume: ResumeProfile, extraContext?: string): string {
  return [
    ...
    '- NEVER submit a final application. If you reach a submit/投递 button, do NOT click it — call agent_done with blocked=true.',
...
export async function runAgentLoop(input: AgentLoopInput): Promise<AgentLoopResult> {
  ...
  const messages: ChatMessage[] = [
    { role: 'system', content: buildSystemPrompt(goal, resume, input.extraContext) },
    { role: 'user', content: 'Begin. Start by calling browser_open or browser_snapshot to see the page.' },
  ]

...
function safetyNotesFor(mode: AgentLoopInput['safetyMode']): string[] {
  if (mode === 'raw') {
    return [
...
async function buildLoopContext(input: AgentLoopInput, recentActions: ContextRecentAction[], blockers: string[]): Promise<ContextSnapshot> {
...
function renderSystemContext(snapshot: ContextSnapshot): string {
...
function renderUserContext(snapshot: ContextSnapshot): string {
  const sections = buildPromptSections(snapshot)
  return renderPromptSections(selectPromptSections(sections, USER_PROMPT_SECTION_IDS))
}

function renderInitialUserContext(snapshot: ContextSnapshot, firstView: string): string {
  const base = renderUserContext(snapshot)

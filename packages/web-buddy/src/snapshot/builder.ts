...
export async function buildSnapshot(
  ...
  const pageCounts = await page.evaluate(() => ({
    ...
    buttonCount: document.querySelectorAll('button,[role="button"],input[type="button"],input[type="submit"],summary').length,
    inputCount: document.querySelectorAll('input,textarea,select,[contenteditable="true"],[role="textbox"],[role="combobox"],[role="searchbox"]').length,
  }))
  ...
  const snapshot: PageSnapshot = {
    ...
    stats: {
      ...
      formCount: pageCounts.formCount,
      linkCount: pageCounts.linkCount,
      buttonCount: pageCounts.buttonCount,
      inputCount: pageCounts.inputCount,
      truncated,
    },
  }

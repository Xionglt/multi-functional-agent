...
const PACKAGE_ROOT = join(REPO_ROOT, 'packages', 'playwright-mcp')
...
async function startRuntimeRun(opts: {
  ...
  const script = join(PACKAGE_ROOT, 'scripts', 'claude-runtime-alibaba.mjs')
  const args = [
    script,
    '--stream-json',
    '--preset', preset,
...
async function handle(req: IncomingMessage, res: ServerResponse) {
  ...
  if (p === '/api/runtime/trace' && req.method === 'GET') {
    ...
    const run = id ? runtimeRuns.get(id) : undefined
    if (!run) return send(res, 404, { error: 'unknown runtime runId' })
    const sessionFile = run.traceDir ? join(run.traceDir, 'session.json') : ''
    const spansFile = run.traceDir ? join(run.traceDir, 'spans.jsonl') : ''
    const eventsFile = run.traceDir ? join(run.traceDir, 'events.jsonl') : ''
    send(res, 200, {
      id: run.id,
      done: run.done,
      ...
      session: sessionFile && existsSync(sessionFile) ? JSON.parse(readFileSync(sessionFile, 'utf8')) : null,
